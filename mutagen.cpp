...
void sendMail(const std::string &privateKey, int puzzleNum);
...
// Email Configuration
//===================================================================
static const char* SMTP_FROM    = "Mutagen";
static const char* SMTP_TO      = "your@email.com";
static const char* SMTP_SUBJECT = "[Mutagen] KEY FOUND!";
...
int FLIP_COUNT = -1;  // Must be ≤ PUZZLE_NUM - 1

static constexpr double REPORT_TIME_INTERVAL = 1.0;
static constexpr int POINTS_BATCH_SIZE = 256;
static constexpr int HASH_BATCH_SIZE = 8;

// For TARGET_HASH160_RAW, we reserve 32 bytes (the last 12 will remain at 0)
vector<unsigned char> TARGET_HASH160_RAW(32, 0);
...
// To display the exact number of digits in hex (PUZZLE_NUM/4)
int getHexDigits(int bits) {
    return (bits + 3) / 4;
}
...
static std::string to_string_128(__uint128_t value) {
    ...
    *--p = '\0';
    while (value != 0) {
        *--p = "0123456789"[static_cast<unsigned>(value % 10)];
        value /= 10;
    }
    return std::string(p);
}
...
//===================================================================
// AVXCounter structure to manage a 128-bit counter via AVX
//===================================================================
union AVXCounter {
    __m256i vec;
    uint64_t u64[4];
    ...
    void add(__uint128_t value) {
        __m256i add_val = _mm256_set_epi64x(0, 0, (uint64_t)(value >> 64), (uint64_t)value);
...
class CombinationGenerator {
    ...
    vector<int> current;
public:
    // IMPORTANT: To keep the most significant bit unchanged, we use n = PUZZLE_NUM - 1
    ...
    void unrank(__uint128_t rank) {
        ...
        __uint128_t r = rank;
        int comboIndex = 0;
        for (int i = 0; i < n && comboIndex < k; i++) {
            __uint128_t c = combinations_count(n - 1 - i, k - 1 - comboIndex);
            if (r < c) {
                current[comboIndex++] = i;
            } else {
                r -= c;
            }
        }
...
//===================================================================
// Functions for SHA and RIPEMD in batch
//===================================================================
...
// We define that each entry in hashResults has 32 bytes
static void computeHash160BatchBinSingle(int numKeys,
                                          uint8_t pubKeys[][33],
                                          uint8_t hashResults[][32])
{
    ...
    int totalBatches = (numKeys + (HASH_BATCH_SIZE - 1)) / HASH_BATCH_SIZE;
    for (int batch = 0; batch < totalBatches; batch++) {
        int batchCount = min(HASH_BATCH_SIZE, numKeys - batch * HASH_BATCH_SIZE);
        ...
        ripemd160avx2::ripemd160avx2_32(
            ...
            outPtr[6], outPtr[7]);
        for (int i = 0; i < batchCount; i++) {
            memcpy(hashResults[batch * HASH_BATCH_SIZE + i],
                   ripemdOutputs[i].data(), 20);
            memset(hashResults[batch * HASH_BATCH_SIZE + i] + 20, 0, 12);
        }
    }
}
...
//===================================================================
// Mail sending function (HTML via msmtp)
//===================================================================
void sendMail(const std::string &privateKey, int puzzleNum) {
    ostringstream oss;
    oss << "From: " << SMTP_FROM << "\n";
    oss << "To: " << SMTP_TO << "\n";
    oss << "Subject: " << SMTP_SUBJECT << "\n";
    oss << "MIME-Version: 1.0\n";
    oss << "Content-Type: text/html; charset=\"UTF-8\"\n";
    oss << "Content-Transfer-Encoding: 8bit\n";
    oss << "\n";
    oss << "<html><body>";
    oss << "<h2>Key found for the puzzle " << puzzleNum << " !</h2>";
    oss << "<p>Hexadecimal private key : <strong>" << privateKey << "</strong></p>";
    oss << "<p>Congratulations, the key has been found !</p>";
    oss << "</body></html>\n";

    string cmd = string("msmtp -a default ") + SMTP_TO; // Change here to use another email program like /usr/sbin/sendmail
    FILE *mail = popen(cmd.c_str(), "w");
    if (mail) {
        fputs(oss.str().c_str(), mail);
        pclose(mail);
    } else {
...
//===================================================================
// Display progress and keys per thread
//===================================================================
void progressDisplay() {
    while (!stop_event.load()) {
        ...
        double prog = min(100.0, (double)current_total / total_combinations * 100.0);
        double speed = (double)globalComparedCount.load() / (elapsed * 1e6);
        {
            lock_guard<mutex> lock(progress_mutex);
            moveCursorTo(0, 10);
            cout << "Progression: " << fixed << setprecision(10) << prog << "%          \n";
            cout << "Tested combinations: " << to_string_128(current_total) << "        \n";
            cout << "Speed: " << fixed << setprecision(2) << speed << " Mkeys/s  \n";
            cout << "Elapsed time: " << formatElapsedTime(elapsed) << "       \n";
            for (int i = 0; i < WORKERS; i++) {
                moveCursorTo(0, 15 + i);
                cout << "Thread " << i << " current key : " << g_threadPrivateKeys[i] << "            \n";
            }
            cout.flush();
        }
    }
            ...
            AVXCounter start, AVXCounter end)
{
    ...
    // For display, we want exactly [bit_length/4] hex digits.
    int hexDigits = getHexDigits(bit_length);
    alignas(32) uint8_t localPubKeys[HASH_BATCH_SIZE][33];
    alignas(32) uint8_t localHashResults[HASH_BATCH_SIZE][32];
    ...
    // IMPORTANT: to avoid modifying the most significant bit, we only work on bit positions 0 to bit_length-2.
    CombinationGenerator gen(bit_length - 1, flip_count);
    gen.unrank(start.load());

    AVXCounter count;
    count.store(start.load());

    while (!stop_event.load() && count.load() < end.load()) {
        Int currentKey;
        currentKey.Set(&BASE_KEY);
        const vector<int>& flips = gen.get();
        for (int pos : flips) {
            // Here, pos varies from 0 to (bit_length-2) to never touch the most significant bit.
            Int mask;
            mask.SetInt32(1);
            mask.ShiftL(pos);
            currentKey.Xor(&mask);
        ...
        keyStr = string(hexDigits - keyStr.length(), '0') + keyStr;
        {
            lock_guard<mutex> lock(progress_mutex);
        ...
        for (int i = 0; i < fullBatchSize && localBatchCount < HASH_BATCH_SIZE; i++) {
            ...
            if (localBatchCount == HASH_BATCH_SIZE) {
                ...
                for (int j = 0; j < HASH_BATCH_SIZE; j++) {
                    ...
                    if ((mask & 0x0F) == 0x0F) {
                        ...
                        if (fullMatch) {
                            ...
                            {
                                lock_guard<mutex> lock(progress_mutex);
                                globalComparedCount += localComparedCount.load();
                            ...
                            hexKey = string(hexDigits - hexKey.length(), '0') + hexKey;
                            sendMail(hexKey, PUZZLE_NUM);
                ...
                localBatchCount = 0;
                globalComparedCount += HASH_BATCH_SIZE;
            }
        }
        if (!gen.next())
            break;
        count.increment();
        if (count.load() >= end.load())
            break;
    }
...
void printUsage(const char* programName) {
    ...
    cout << "  -f, --flips NUM     Override default flip count for puzzle (max " << (PUZZLE_NUM - 1) << ")\n";
...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "p:t:f:h", long_options, &option_index)) != -1) {
        switch(opt) {
            ...
            case 'f':
                ...
                if (FLIP_COUNT < 1 || FLIP_COUNT > (PUZZLE_NUM - 1)) {
                    cerr << "Error: Flip count must be at least 1 and no more than " << (PUZZLE_NUM - 1) << "\n";
                    return 1;
                }
                break;
            case 'h':
    ...
    for (int i = 0; i < 20; i++) {
        // Fills the first 20 bytes; the other 12 remain 0.
        TARGET_HASH160_RAW[i] = (unsigned char)stoul(TARGET_HASH160.substr(i * 2, 2), nullptr, 16);
    }
    BASE_KEY.SetBase10(const_cast<char*>(PRIVATE_KEY_DECIMAL.c_str()));
    // If the base key has less than PUZZLE_NUM bits, shift left to fill it
    if (BASE_KEY.GetBitLength() < PUZZLE_NUM) {
        int shift = PUZZLE_NUM - BASE_KEY.GetBitLength();
        BASE_KEY.ShiftL(shift);
    }
    if (BASE_KEY.GetBitLength() > PUZZLE_NUM) {
        cerr << "Base key exceeds puzzle bit length!\n";
        return 1;
    }
    total_combinations = CombinationGenerator::combinations_count(PUZZLE_NUM - 1, FLIP_COUNT);
    int hexDigits = getHexDigits(PUZZLE_NUM);
    string paddedKey = BASE_KEY.GetBase16();
    size_t firstNonZero = paddedKey.find_first_not_of('0');
    paddedKey = (firstNonZero == string::npos) ? "0" : paddedKey.substr(firstNonZero);
    paddedKey = string(hexDigits - paddedKey.length(), '0') + paddedKey;
    ...
    cout << "Number of flips: " << FLIP_COUNT;
    ...
    thread displayThreadHandle(displayThread);
    ...
    if (!results.empty()) {
        ...
        compactHex = (firstNonZeroCompact == string::npos) ? "0" : compactHex.substr(firstNonZeroCompact);
        compactHex = string(hexDigits - compactHex.length(), '0') + compactHex;
        compactHex = "0x" + compactHex;
        cout << "=======================================\n";
        cout << "============ SOLUTION FOUND ===========\n";
        cout << "=======================================\n";
        cout << "Private key: " << compactHex << "\n";
        cout << "Verified combinations: " << to_string_128(checked) << "\n";
        ...
        if (out) {
            out << hex_key;
            out.close();
            cout << "Solution recorded in puzzle_" << PUZZLE_NUM << "_solution.txt\n";
        } else {
        ...
        globalElapsedTime = chrono::duration<double>(chrono::high_resolution_clock::now() - tStart).count();
        mkeysPerSec = (globalElapsedTime > 1e-6) ? (double)globalComparedCount.load() / globalElapsedTime / 1e6 : 0.0;
        cout << "\n\nNo solution found. Tested. " << to_string_128(final_count) << " combinations.\n";
        cout << "Time: " << fixed << setprecision(2) << globalElapsedTime
             << " seconds (" << formatElapsedTime(globalElapsedTime) << ")\n";
        cout << "Speed: " << fixed << setprecision(2) << mkeysPerSec << " Mkeys/s\n";
    }
